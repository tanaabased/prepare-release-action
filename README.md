...
| `sync-verified` | A toggle to commit synced changes via GitHub REST API for bot-verified commits. | `false` | `true` |
| `sync-message` | The commit message to use when syncing changes back to the repo. | `release v%s generated by @tanaabased/prepare-release-action` | `RELEASE %s` |
...
When `sync-verified=true`, this action creates the sync commit with the GitHub REST API (instead of local `git commit`) and then pushes tags as usual.
