...
var ANSI_PATTERN = new RegExp(String.raw`\u001B\[[0-9;]*m`, "g");
var CLI_NAME = "version-injector";
var CSI = "\x1B[";
var DEBUG_NAMESPACE = "version-injector";
var here = path.dirname(fileURLToPath(import.meta.url));
var packageJsonPath = path.join(here, "..", "package.json");
var packageJson = JSON.parse(await fs.readFile(packageJsonPath, "utf8"));
var validInsertions = new Set(["after-shebang", "top", "bottom"]);
var validStyles = new Set(["js", "sh", "ps1"]);
var supportsColor = (stream = process.stdout) => {
...
var applyAnsi = (code, text, stream = process.stdout) => {
...
var applyRgb = (hex, text, stream = process.stdout) => {
  const value = String(text);
  if (!supportsColor(stream)) {
    return value;
  }
...
var stripAnsi = (value) => String(value).replaceAll(ANSI_PATTERN, "");
var bold = (text, stream = process.stdout) => applyAnsi("1", text, stream);
var dim = (text, stream = process.stdout) => applyAnsi("2", text, stream);
var green = (text, stream = process.stdout) => applyAnsi("32", text, stream);
var red = (text, stream = process.stdout) => applyAnsi("31", text, stream);
var tp = (text, stream = process.stdout) => applyRgb("#00c88a", text, stream);
var ts = (text, stream = process.stdout) => applyRgb("#db2777", text, stream);
var writeLine = (stream, message = "") => {
  stream.write(`${message}
`);
};
var writeStatus = (stream, label, colorize, message = "") => {
...
var debugPatternMatches = (pattern, namespace) => {
  if (!pattern) {
    return false;
  }
...
var envDebugEnabled = (namespace) => {
  ...
  return rawDebug.split(",").map((segment) => segment.trim()).some((pattern) => debugPatternMatches(pattern, namespace));
};
var debugLog = (enabled, message = "") => {
  if (!enabled) {
    return;
  }
...
var getOptionValue = (args, index, token) => {
...
var buildEnvironment = () => Object.freeze({
  ...
  versionValue: process.env.VERSION_INJECTOR_VERSION === undefined ? null : String(process.env.VERSION_INJECTOR_VERSION)
});
var resolveInvocation = (parsedArgs) => {
...
var formatHelpEntries = (entries) => {
  const width = entries.reduce((maxWidth, entry) => Math.max(maxWidth, stripAnsi(entry.label).length), 0);
...
var renderHelp = () => {
  ...
  const options = [
    ...
    {
      label: "--version",
      description: `shows the CLI version ${dim(`[default: ${packageJson.version}]`)}`
    }
  ];
  return [
    `Usage: ${bold(`${CLI_NAME} <file> --style <js|sh|ps1> --version <value> [options]`)}`,
    "",
...
var planUpdate = (content, options) => {
  ...
  debugLog(options.debug, `matched ${matches.length} candidate line${matches.length === 1 ? "" : "s"} in ${options.file}`);
  if (matches.length > 1) {
    fail(`Found multiple ${options.name} assignments or placeholders in ${options.file}; refusing to choose one.`);
  }
  ...
  if (matches.length === 1) {
    ...
    fail(`Could not find an existing ${options.name} assignment or placeholder in ${options.file}.`);
  }
...
var invocation = resolveInvocation(parsedArgs);
if (invocation.help) {
  log(renderHelp());
  process.exit(0);
}
if (invocation.showCliVersion && invocation.file === null && parsedArgs.style === null && parsedArgs.versionValue === null && parsedArgs.insert === null && parsedArgs.name === null && parsedArgs.check === false && parsedArgs.dryRun === false && parsedArgs.debug === null) {
  log(packageJson.version);
  process.exit(0);
}
if (invocation.showCliVersion) {
  fail("Bare --version only prints the CLI version when no file path or injection options are provided.");
}
validateArgs(invocation);
var targetPath = path.resolve(invocation.file);
debugLog(invocation.debug, `resolved file=${targetPath} style=${invocation.style} name=${invocation.name} insert=${displayValue(invocation.insert)} check=${invocation.check} dry-run=${invocation.dryRun}`);
var content;
try {
...
var result = planUpdate(content, { ...invocation, file: targetPath });
if (invocation.check) {
  if (result.changed) {
...
if (invocation.dryRun) {
  note(result.changed ? `${tp("update")} ${ts(targetPath)} ${dim("(dry run)")}` : `${tp("skip")} ${ts(targetPath)} ${dim("(already matches)")}`);
  process.exit(0);
}
if (!result.changed) {
...
//# debugId=A2B5B0CED316366364756E2164756E21
